...
function getRowData(share, callback){
    ...
    async.parallel([
        ...
        function(err, result){
            ...
            console.log(rowDataArray.length);
            callback(null);
        })
}
...
function addColumn(){
    ...
    sqlSentence = 'ALTER TABLE share_name_code_date ADD COLUMN status tinyint(1) not null;';

    connectionData.query(sqlSentence,function(err){
        if(err){
            console.log("ADD COLUMN ERROR", err.message);
        }
    });
}

function initShareStatus(){

    sqlSentence = 'UPDATE share_name_code_date SET status = 0;';

    connectionData.query(sqlSentence,function(err){
        if(err){
            console.log("UPDATE share_name_code_date ERROR", err.message);
        }
    });
}
